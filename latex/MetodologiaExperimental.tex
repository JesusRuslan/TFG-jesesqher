...
\begin{abstract}
Este capítulo describe el diseño de los experimentos realizados para comparar los algoritmos de planificación de trayectorias, incluyendo la selección de objetivos, el protocolo de ejecución automatizado y las limitaciones del entorno de simulación que condicionan los resultados.
\end{abstract}

\section{Diseño del experimento}

El objetivo de los experimentos es comparar el comportamiento de distintos algoritmos de planificación de trayectorias en las mismas condiciones, sobre el mismo mapa y con los mismos puntos de origen y destino. Para garantizar la validez de la comparativa, todos los experimentos parten de la misma posición inicial del robot y utilizan los mismos objetivos en el mismo orden.

\subsection{Selección de objetivos}

Se han definido cuatro objetivos de navegación con características deliberadamente distintas, con el fin de evaluar los algoritmos en situaciones variadas:
...
\begin{itemize}
    ...
    \item \textbf{goal4\_compleja} $(-13, 19)$: trayectoria larga con múltiples obstáculos y cambios de dirección. Es el caso más exigente y donde se esperan las mayores diferencias entre algoritmos. Este objetivo se utiliza también como único destino en el nodo de métricas dinámico.
\end{itemize}

Adicionalmente, se incluye un objetivo de calentamiento $(-1, 0)$ al inicio de cada ejecución, cuyas métricas no se registran. Su propósito es forzar la inicialización de las cachés internas de Nav2, de forma que los tiempos de planificación de los objetivos reales sean comparables entre sí.
...
\begin{enumerate}
    ...
    \item A \textbf{t=30\,s:} arranca el nodo de métricas, que envía los objetivos y recoge los datos automáticamente.
\end{enumerate}

Al finalizar todos los objetivos, el nodo guarda el CSV y se cierra. El proceso completo no requiere ninguna interacción del usuario.

\section{Limitaciones del entorno de simulación}
\label{sec:limitaciones_simulacion}
