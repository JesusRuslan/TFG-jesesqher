...
\begin{quotation}[Informático]{Linus Torvalds (1969--)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/graphs/ros2_node_graph_turtlebot4_final}
    \caption{Grafo de nodos ROS2 del nodo \texttt{turtlebot4\_first\_cpp\_node}: suscripciones a \texttt{/interface\_buttons} y \texttt{/odom}, y publicación en \texttt{/cmd\_vel}.}
...
El nodo \texttt{data\_measure} implementa un action client de Nav2 que automatiza el envío de objetivos de navegación y la recogida de métricas sobre la trayectoria planificada. Se denomina ``estático'' porque cancela la navegación inmediatamente tras recibir el plan, sin que el robot ejecute físicamente la trayectoria.

\subsection{Objetivos de navegación}

El nodo itera sobre un conjunto de cuatro objetivos de navegación distintos definidos como coordenadas fijas en el mapa del almacén, seleccionados para evaluar el comportamiento en diferentes trayectorias. Además, se incluye un objetivo genérico inicial para precargar la memoria del planificador.

\begin{itemize}
    \item \textbf{warmup} $(-1, 0)$: objetivo de calentamiento, no se registran sus métricas. Necesario porque Nav2 inicializa cachés internas en la primera planificación, lo que hace que el primer objetivo tarde significativamente más que los siguientes.
...
El nodo sigue el siguiente flujo para cada objetivo:

\begin{enumerate}
    \item Envía el objetivo a Nav2 mediante \texttt{async\_send\_goal} y anota el timestamp de envío.
    ...
    \item Guarda las métricas en el CSV y espera 3 segundos antes de enviar el siguiente objetivo.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/graphs/flujo_nodo_estatico}
    \caption{Flujo de funcionamiento del nodo de métricas estático para cada objetivo de navegación.}
    \label{fig:flujo_nodo_estatico}
\end{figure}

\subsection{Métricas calculadas}

Para cada objetivo se calculan las siguientes métricas:

\begin{itemize}
    \item \textbf{Tiempo de planificación:} diferencia entre el timestamp de envío del objetivo y el de recepción del plan, en milisegundos.
...
El nodo \texttt{data\_measure\_dynamic} complementa al nodo estático recogiendo métricas durante la ejecución real de la navegación. A diferencia del nodo estático, el robot sí navega hasta el destino, lo que permite medir cómo se ejecuta la trayectoria en la práctica.

\subsection{Objetivos de navegación}

El nodo procesa únicamente dos objetivos: un warmup $(-1, 0)$ y el objetivo dinámico $(-13, 19)$, que corresponde al objetivo más complejo del nodo estático y ofrece suficiente recorrido para observar el comportamiento del robot durante un tiempo prolongado.

\subsection{Flujo de funcionamiento}

Para el objetivo de warmup, el nodo envía el objetivo y espera a que Nav2 lo complete sin registrar datos. Al completarse, limpia el buffer de muestras y el plan almacenado antes de proceder al objetivo real.

Para el objetivo dinámico, activa el muestreo a 5\,Hz desde el momento en que envía el objetivo hasta que Nav2 reporta que el robot ha llegado al destino. En cada muestra se busca el punto del plan más cercano a la posición real del robot para calcular la desviación. Al finalizar, vuelca todas las muestras al CSV.
