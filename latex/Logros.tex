...
\begin{quotation}[Escritor]{T. S. Eliot (1888--1965)}
El fin de toda nuestra exploración será llegar a donde empezamos y conocer el lugar por primera vez.
\end{quotation}
...
\begin{itemize}
    ...
    \item El suavizado de trayectorias (\texttt{do\_refinement: true}) tiene un efecto limitado sobre la irregularidad (reducción inferior al 5\%) pero mejora moderadamente la curvatura media (entre un 10 y un 20\% en la mayoría de los objetivos). Su coste en tiempo de planificación es de entre 3 y 9 ms en Dijkstra.
    ...
    \item Dijkstra smooth es la configuración con mejor desviación media global (1,204 m) y la que produce un comportamiento de navegación más estable y predecible, siendo la opción recomendada para entornos de almacén con obstáculos estáticos. No obstante, estos resultados deben interpretarse con cautela: como se ha señalado en el análisis de trayectorias, la reconstrucción del plan desde el CSV presenta discontinuidades debidas a replanificaciones de Nav2, lo que introduce un error sistemático en la métrica de desviación respecto al plan.
\end{itemize}
